...
//  -------------------------------------------------------------------- WOFF Methods
    ...
    protected static function woffReadHeader($fh): array
    {
        ...
        switch ($header['flavor']) {
            case 0x00010000:$header['type'] = 'ttf';
                break;
            case 0x4F54544F:$header['type'] = 'cff';
                break;
            default:$header['type'] = 'xxx';
                break;
        }
        if (self::$debug) {
            self::debug("WOFF Flavor", $header['type']);
        }
    ...
    protected static function woffReadTableDir($fh, int $numTables): array
    {
        ...
        if (self::$debug) {
            self::debug("WOFF Table directory", $entries);
        }
    ...
    protected static function woffReadFontTables($fh, array &$entries): array
    {
        ...
        if (self::$debug) {
            self::debug("WOFF Font tables", \array_map('strlen', $tables));
        }
    ...
    protected static function woffReadPrivData($fh, int $offset, int $length): string
    {
        ...
        if (self::$debug) {
            self::debug("WOFF protected data", $privData);
        }
...
//  -------------------------------------------------------------------- TTF Methods
    ...
    protected static function ttfWriteTableDir($fh, array $entries): int
    {
        ...
        if ($c === self::TTF_TableDirSize * \count($entries)) {
            return $c;
        }
    ...
    protected static function ttfWriteFontTables($fh, array $entries, array &$tables): int
    {
        ...
        foreach ($entries as $n => $entry) {
            ...
            if ($c !== $s) {
                self::error(__METHOD__, "TTF output error: failed to write font table ($n: $c/$s)");
            }
...
//  -------------------------------------------------------------------- Public Methods
    ...
    {
        if (null === $ttfFile) {
            $ttfFile = \sprintf('%s/%s.ttf', \dirname($woffFile), \basename($woffFile, '.woff'));
        }
